// Load Atenxion Widget on all pages
(function () {
  const atenxionScript = document.createElement("script");
  atenxionScript.src =
    "https://qa.atenxion.ai/api/atenxion-widget-script?agentchainId=690c1c4bd7689db5d7f83c98";
  atenxionScript.async = true;
  document.head.appendChild(atenxionScript);
})();
...
async function init() {
  await Promise.all([
    ...
    loadJSON("/data/departments.json").then(
    ...
    loadJSON("/data/research_centers.json").then(
      (data) => (state.researchCenters = data)
    ),
    loadJSON("/data/intakes.json").then((data) => (state.intakes = data)),
    loadJSON("/data/admissions_requirements.json").then(
      (data) => (state.admissionsRequirements = data)
    ),
  ]);
...
function renderHome() {
  ...
  if (statsGrid && state.university) {
    const stats = [
      ...
      {
        label: "Students",
        value: state.university.snapshot.studentCount.toLocaleString(),
      },
      {
        label: "Faculties & Schools",
        value: state.university.snapshot.facultiesCount,
      },
      {
        label: "Full-time Staff",
        value: state.university.snapshot.fullTimeStaff.toLocaleString(),
      },
    ];
...
function renderAcademics() {
  ...
  if (facultySelect) {
    facultySelect.innerHTML += state.faculties
      .map(
        (faculty) => `<option value="${faculty.id}">${faculty.name}</option>`
      )
      .join("");
  }
  if (facultyAccordion) {
    facultyAccordion.innerHTML = state.faculties
      .map((faculty) => {
        const departments = state.departments.filter(
          (dept) => dept.facultyId === faculty.id
        );
        const sampleCourses = state.courses
          ...
          <div class="accordion-body">
            ...
            <ul>
              ${departments
                .map(
                  (dept) =>
                    `<li><strong>${dept.name}</strong> — Chair: ${dept.chairName}. ${dept.description}</li>`
                )
                .join("")}
            </ul>
            ...
            <ul>
              ${sampleCourses
                .map(
                  (course) =>
                    `<li><a href="/course.html?code=${course.courseCode}">${course.title}</a></li>`
                )
                .join("")}
            </ul>
          </div>
...
function setupCourseSearch() {
  ...
  const renderResults = (list) => {
    if (list.length === 0) {
      results.innerHTML =
        "<p>No courses match your filters yet. Try another search.</p>";
      return;
    }
        ...
        const cycles = intake.cycles
          .map(
            ...
            <li>
              ...
              Application fee: ${cycle.applicationFee}${
              cycle.decisions
                ? `<br><span class="cycle-note">Decisions: ${cycle.decisions}</span>`
                : ""
            }${
              cycle.orientation
                ? `<br><span class="cycle-note">Orientation: ${cycle.orientation}</span>`
                : ""
            }
            </li>`
          )
    ...
    requirementsGrid.innerHTML = state.admissionsRequirements
      .map(
        ...
        <article class="card">
          ...
          <p><strong>English proficiency options:</strong> ${req.englishTests.join(
            ", "
          )}</p>
...
function renderResearch() {
  ...
  centerGrid.innerHTML = state.researchCenters
    .map((center) => {
      const lead = state.lecturers.find(
        (lecturer) => lecturer.id === center.leadLecturerId
      );
      return `
      <article class="card">
        <h3>${center.name}</h3>
    ...
    newsCards.innerHTML = state.news
      .map(
        ...
        <article class="card news-card">
          ...
          <h3><a href="/news/article.html?slug=${item.slug}">${
          item.title
        }</a></h3>
          <p>${item.summary}</p>
          <p class="news-card-meta">By ${item.author} · ${item.readTime}</p>
          <a class="card-link" href="/news/article.html?slug=${
            item.slug
          }">Read story</a>
        </article>`
      )
...
function renderCourseDetail() {
  ...
  document.getElementById(
    "course-credits"
  ).textContent = `${course.credits} credits`;
  document.getElementById(
    "course-duration"
  ).textContent = `${course.duration} semesters`;
  ...
  document.getElementById(
    "course-tuition"
  ).textContent = `$${course.tuitionPerYear.toLocaleString()}`;
  document.getElementById("course-intakes").textContent =
    course.intakeSeasons.join(", ");
  document.getElementById("course-prerequisites").textContent = course
  ...
  document.getElementById("course-curriculum").innerHTML =
    course.curriculumOutline.map((module) => `<li>${module}</li>`).join("");
  ...
  document.getElementById("related-courses").innerHTML = related
    .map(
      (item) =>
        `<li><a href="/course.html?code=${item.courseCode}">${item.title}</a></li>`
    )
    .join("");
}
...
function renderLecturerDetail() {
  ...
  document.getElementById(
    "lecturer-photo"
  ).alt = `Portrait of ${lecturer.fullName}`;
  document.getElementById(
    "lecturer-contact"
  ).innerHTML = `${lecturer.email} · ${lecturer.phone} · ${lecturer.office}`;
  document.getElementById("lecturer-bio").textContent = lecturer.shortBio;
  document.getElementById("lecturer-research").innerHTML =
    lecturer.researchAreas.map((area) => `<li>${area}</li>`).join("");
  document.getElementById("lecturer-publications").innerHTML =
    lecturer.publications
      .map((publication) => `<li>${publication}</li>`)
      .join("");
  const teachingCourses = state.courses.filter((course) =>
    course.lecturers?.includes(lecturer.id)
  );
